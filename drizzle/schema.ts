...
export const stocks = mysqlTable("stocks", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({ sectorIdx: index("stocks_sector_idx").on(table.sector) }));
...
export const marketData = mysqlTable("market_data", {
  ...
  volume: int("volume").notNull(),
}, (table) => ({ stockDateIdx: index("market_data_stock_date_idx").on(table.stockId, table.timestamp) }));
...
export const holdings = mysqlTable("holdings", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({ uniqueHolding: uniqueIndex("holdings_portfolio_stock_unique").on(table.portfolioId, table.stockId) }));
...
export const orders = mysqlTable("orders", {
  ...
  orderType: mysqlEnum("orderType", ["MARKET"]).default("MARKET").notNull(),
  quantity: decimal("quantity", { precision: 14, scale: 4 }).notNull(),
...
export const transactions = mysqlTable("transactions", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({ userCreatedIdx: index("transactions_user_created_idx").on(table.userId, table.createdAt) }));
...
export const watchlists = mysqlTable("watchlists", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({ userNameUnique: uniqueIndex("watchlists_user_name_unique").on(table.userId, table.name) }));
...
export const watchlistItems = mysqlTable("watchlist_items", {
  ...
  stockId: int("stockId").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({ uniqueItem: uniqueIndex("watchlist_items_watchlist_stock_unique").on(table.watchlistId, table.stockId) }));
...
export const news = mysqlTable("news", {
  ...
  sector: varchar("sector", { length: 80 }).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({ publishedIdx: index("news_published_idx").on(table.publishedAt), sectorIdx: index("news_sector_idx").on(table.sector) }));
...
export const alerts = mysqlTable("alerts", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({ userActiveIdx: index("alerts_user_active_idx").on(table.userId, table.isActive) }));
...
export const portfolioSnapshots = mysqlTable("portfolio_snapshots", {
  ...
  timestamp: timestamp("timestamp").notNull(),
}, (table) => ({ portfolioTimeIdx: index("snapshots_portfolio_time_idx").on(table.portfolioId, table.timestamp) }));
